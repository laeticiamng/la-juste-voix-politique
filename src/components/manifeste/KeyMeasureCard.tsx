  ...
  return (
    <div className="bg-card rounded-xl shadow-[var(--ljv-shadow-card)] border border-border overflow-hidden hover:shadow-[var(--ljv-shadow-elevated)] transition-all duration-300 hover-scale">
      <div className="p-6">
        ...
        <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3">{measure.title}</h3>
        
        {measure.description && (
          <p className="text-gray-800 dark:text-gray-200 mb-3 leading-relaxed font-medium">{measure.description}</p>
        )}
        
        {measure.slogan && (
          <blockquote className="border-l-4 border-ljv-gold pl-4 italic text-gray-800 dark:text-gray-200 mb-3 bg-ljv-gold/5 py-2 rounded-r font-medium">
            "{measure.slogan}"
          </blockquote>
        )}
        
        {measure.examples && (
          <ul className="list-disc pl-6 mb-3 text-gray-800 dark:text-gray-200 space-y-1.5 leading-relaxed font-medium">
        ...
        {measure.strengths && (
          <div className="mt-4 bg-green-50 dark:bg-green-950/20 rounded-lg p-4 border border-green-200 dark:border-green-800">
            <p className="font-bold text-gray-900 dark:text-gray-100 mb-2">Pourquoi c'est fort :</p>
            <ul className="list-none space-y-2">
              {measure.strengths.map((strength, index) => (
                <li key={index} className="flex items-start gap-2 text-gray-900 dark:text-gray-100">
                  ...
                  <span className="pt-0.5 leading-relaxed font-medium">{strength}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {measure.note && (
          <div className="mt-3 text-gray-800 dark:text-gray-200 italic bg-accent/50 rounded p-3 leading-relaxed font-medium">
            {measure.note}
          </div>
        )}
      </div>
